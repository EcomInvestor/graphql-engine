...
- **Authentication** is handled outside of Hasura. Hasura delegates authentication and resolution of request headers
  ...
  `X-Hasura-admin`, etc. More often than not, you'll also need to pass user information for your access control use
  cases, like `X-Hasura-admin-IdX-Hasura-Role`, to build permission rules.
